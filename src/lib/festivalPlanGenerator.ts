...
export interface FestivalData {
	...
	type?: string;
	visitorCount: string;
	customStations?: StationAssignment[];
	customShifts?: ShiftData[];
}
...
export async function generateFestivalPlan(festivalData: FestivalData, members: unknown[] = []) {
	...
	const [aiStations, aiShifts, aiInsights] = await Promise.all([
		aiService.generateFestivalStations(festivalData),
		aiService.generateFestivalShifts(festivalData),
		aiService.generateFestivalInsights(festivalData)
	]);
...
function generateChecklist(festivalData: FestivalData): ChecklistItem[] {
	const baseChecklist = [
		// 6 Monate vorher
		{
			task: 'Genehmigung beim Amt beantragen',
			category: 'Behörden',
			monthsOffset: -6,
			priority: 'red' as const
		},
		{
			task: 'Versicherung abschließen',
		...
		// 2 Monate vorher
		{
			task: 'Zelt und Bühne organisieren',
			category: 'Ausstattung',
			monthsOffset: -2,
			priority: 'yellow' as const
		},
		{
			task: 'Musik/DJ buchen',
			category: 'Entertainment',
			monthsOffset: -2,
			priority: 'green' as const
		},
		{
			task: 'Lieferanten kontaktieren',
		...
		// 2 Wochen vorher
		{
			task: 'Flyer und Plakate drucken',
			category: 'Marketing',
			monthsOffset: -0.5,
			priority: 'green' as const
		},
		{
			task: 'Getränke bestellen',
			category: 'Verpflegung',
			monthsOffset: -0.5,
			priority: 'yellow' as const
		},
		...
		// 1 Woche vorher
		{
			task: 'Helfer einteilen',
			category: 'Personal',
			monthsOffset: -0.25,
			priority: 'red' as const
		},
		{
			task: 'Sicherheitsdienst organisieren',
			category: 'Sicherheit',
			monthsOffset: -0.25,
			priority: 'red' as const
		},
		...
		// Nachbereitung
		{
			task: 'Aufräumen und Abbau',
			category: 'Nachbereitung',
			monthsOffset: 0.003,
			priority: 'yellow' as const
		},
		{
			task: 'Abrechnung erstellen',
			category: 'Nachbereitung',
			monthsOffset: 0.1,
			priority: 'green' as const
		}
...
function getFestivalSpecificTasks(type: string) {
	const tasks: {
		...
		feuerwehr: [
			{
				task: 'Feuerwehrauto als Deko',
				category: 'Dekoration',
				monthsOffset: -0.25,
				priority: 'green'
			},
			{
				task: 'Uniformen für Helfer',
				category: 'Ausstattung',
				monthsOffset: -0.5,
				priority: 'yellow'
			}
		...
		kirtag: [
			{
				task: 'Karussell organisieren',
				category: 'Attraktionen',
				monthsOffset: -2,
				priority: 'yellow'
			},
			{
				task: 'Würstelbude aufstellen',
				category: 'Verpflegung',
				monthsOffset: -0.25,
				priority: 'green'
			}
		...
		wein: [
			...
			{
				task: 'Weinverkostung vorbereiten',
				category: 'Verpflegung',
				monthsOffset: -0.25,
				priority: 'yellow'
			}
		...
		weihnachten: [
			{
				task: 'Weihnachtsdeko besorgen',
				category: 'Dekoration',
				monthsOffset: -1,
				priority: 'yellow'
			},
			{
				task: 'Glühweinstand aufbauen',
				category: 'Verpflegung',
				monthsOffset: -0.25,
				priority: 'red'
			}
...
function getVisitorSpecificTasks(visitorCount: string) {
	const tasks: {
		...
		xlarge: [
			{
				task: 'Sicherheitskräfte aufstocken',
				category: 'Sicherheit',
				monthsOffset: -0.5,
				priority: 'red'
			},
			{
				task: 'Notausgang markieren',
				category: 'Sicherheit',
				monthsOffset: -0.25,
				priority: 'red'
			},
...
function generateStations(festivalData: FestivalData): StationAssignment[] {
	...
	return allStations.map((station, index) => {
		...
		return {
			...
			priority: station.priority as 'green' | 'yellow' | 'red'
		};
	});
}

function getFestivalSpecificStations(type: string) {
	const stations: { [key: string]: Array<{ bereich: string; bedarf: number; priority: string }> } =
		{
			feuerwehr: [{ bereich: 'Feuerwehrshow', bedarf: 4, priority: 'green' }],
			musik: [
			...
			wein: [{ bereich: 'Weinverkostung', bedarf: 3, priority: 'yellow' }],
			weihnachten: [
...
function getVisitorSpecificStations(visitorCount: string) {
	const stations: { [key: string]: Array<{ bereich: string; bedarf: number; priority: string }> } =
		{
			large: [{ bereich: 'Parkplatz', bedarf: 2, priority: 'yellow' }],
			xlarge: [
	...
	const baseResources = [
		{
			...
			menge: '8',
			einheit: 'Stück',
			lieferant: 'Brauerei Stainz',
			kosten: '€ 800',
			priority: 'green' as const
		},
		{
			item: 'Garnituren (Tisch + 2 Bänke)',
			menge: '15',
			einheit: 'Sets',
			lieferant: 'Zeltverleih Graz',
			kosten: '€ 300',
			priority: 'yellow' as const
		},
		{
...
function getVisitorSpecificResources(visitorCount: string) {
	const resources: {
		...
		large: [
			{
				item: 'Mobile Toiletten',
				menge: '4',
				einheit: 'Stück',
				lieferant: 'Sanitär Service',
				kosten: '€ 400',
				priority: 'red'
			},
			{
				item: 'Zusätzliches Zelt',
				menge: '1',
				einheit: 'Stück',
				lieferant: 'Zeltverleih Graz',
				kosten: '€ 600',
		...
		xlarge: [
			{
				item: 'Sicherheitszäune',
				menge: '50',
				einheit: 'Meter',
				lieferant: 'Absperrtechnik',
				kosten: '€ 300',
				priority: 'red'
			},
			{
				item: 'Notausgang-Schilder',
				menge: '6',
				einheit: 'Stück',
				lieferant: 'Sicherheit Plus',
				kosten: '€ 60',
				priority: 'red'
			}
		]
...
// Removed - now using real Mistral AI integration
...
function getFestivalSpecificShiftStations(type: string) {
	const stations: {
		[key: string]: Array<{ name: string; required_people: number; description: string }>;
	} = {
		feuerwehr: [
			{
				name: 'Feuerwehrshow',
				required_people: 4,
				description: 'Präsentation der Feuerwehraktivitäten'
			}
		],
		musik: [
...
function getVisitorSpecificShiftStations(visitorCount: string) {
	const stations: {
		...
		large: [{ name: 'Parkplatz', required_people: 2, description: 'Parkplatzeinweisung' }],
		xlarge: [
