  ...
  try {
    ...
    for (const position of assignmentMatrix) {
      const availableMembers = members.filter(member => {
        ...
        return (
          !alreadyAssignedToShift &&
          currentShifts < config.maxShiftsPerMember &&
          member.is_active
        );
      });
      ...
      const memberScores: AssignmentScore[] = availableMembers.map(member => {
        ...
        const hasPreference = config.respectPreferences && 
          member.station_preferences?.includes(position.stationId);
        if (hasPreference) {
          score += 1000;
        }
