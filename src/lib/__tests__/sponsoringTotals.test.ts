...
function makeCategory(name: string, value: number | null): SponsoringCategory {
	idCounter += 1;
	return {
		id: `cat-${idCounter}`,
		festival_id: 'f1',
...
function makeAssignment(opts: {
	...
	const category = opts.category ?? makeCategory(opts.categoryName ?? 'Kategorie', opts.categoryValue ?? null);
	idCounter += 1;
	return {
		id: `as-${idCounter}`,
		sponsoring_id: 'spo-1',
