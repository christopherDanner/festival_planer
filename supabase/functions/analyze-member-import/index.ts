...
${sampleRows.slice(0, 3).map((row, i) => `Zeile ${i + 1}: ${JSON.stringify(row)}`).join('\n')}
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'system',
            content: 'Du bist ein Experte für Datenanalyse und hilfst bei der automatischen Erkennung von Spaltentypen in Mitgliederdaten. Antworte immer mit gültigem JSON.'
          },
          {
            role: 'user',
            content: prompt
...
function createFallbackAnalysis(headers: string[], sampleRows: any[][]): AnalysisResponse {
  ...
  headers.forEach(header => {
    ...
    } else if (lowerHeader.includes('vorname') || lowerHeader.includes('firstname') || lowerHeader === 'first') {
      mappings[header] = {
        field: 'firstName',
        confidence: 0.85,
        reasoning: 'Header deutet auf Vorname hin'
      };
    } else if (lowerHeader.includes('nachname') || lowerHeader.includes('lastname') || lowerHeader === 'last') {
      mappings[header] = {
        field: 'lastName',
        confidence: 0.85,
